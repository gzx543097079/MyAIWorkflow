...
1. 读取 `references/planning/project-status.md` 判断当前版本，或帮助用户选择应该进入的版本。
2. 读取最相关、最小范围的参考文件。
3. 说明当前版本目标和完成标准。
4. 规划或实现一个最小可用的下一步改动。
...
- `references/development/code-standards.md`
- `references/development/ui-standards.md`
...
- `references/planning/`：版本路线、当前状态、历史进度快照、下一步规划。
- `references/development/`：架构、代码规范、UI 规范。
- `references/quality/`：发布前检查、质量验收。
- `references/prompts/`：可复用 AI 提示词。

当用户询问这些内容时，读取 `references/planning/version-roadmap.md`：
...
当用户询问当前进度、下一步、从哪里继续时，优先读取 `references/planning/project-status.md`。

当用户询问这些内容时，读取 `references/development/flutter-architecture.md`：
...
当用户询问这些内容时，读取 `references/prompts/prompt-templates.md`：
...
当用户询问这些内容时，读取 `references/quality/release-checklist.md`：
