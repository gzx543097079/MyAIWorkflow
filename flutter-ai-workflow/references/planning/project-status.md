...
| V0.9 体验优化版 | 已完成 | 已支持月份切换、错误提示、删除反馈和深色模式 |
| V1.0 发布候选版 | 已完成 | 核心流程、双端构建运行、持久化和金额计算回归验证已完成 |
...
- App 状态控制器加入错误状态，并提供重试和关闭入口。
- Repository 加载或操作失败时会显示用户可见错误提示。
- 删除记录后会显示完成提示。
- App 支持深色模式。
- 设置页加入深色模式开关。
- iOS Runner 项目补充 `iphonesimulator` 支持平台配置。
- 补充状态控制器测试，覆盖月份筛选、深色模式和错误状态。
- 补充 widget 测试，覆盖深色模式切换。
- 更新设置页版本名为 `V0.9 体验优化版`。
...
完成内容：
- 更新 App 版本名为 `V1.0 发布候选版`。
- 更新 README，补充 V1.0 发布候选功能、已知限制、本地数据说明和下一版本方向。
...
V1.0 之后建议先做真实设备发布前回归。

优先任务：
1. 在真实 iPhone 上验证新增、删除、分类管理、月份切换和深色模式。
2. 在真实 Android 设备上做同样验证。
3. 评估交易编辑、导出、备份或云同步。
4. 根据真实使用反馈继续打磨筛选和统计。
